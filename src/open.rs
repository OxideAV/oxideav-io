...
pub fn open_with(ctx: &RuntimeContext, src: Source, opts: &OpenOptions) -> Result<Opened> {
    ...
    // PDF and 3D models decode to a `Scene` / `Scene3D` rather than
    // through the codec registry. Those eager paths live in the umbrella
    // build (they need the `oxideav-pdf` / `oxideav-mesh3d` crates, which
    // pull the meta fleet); this standalone facade reports them as
    // unsupported so the caller routes them explicitly.
    if is_pdf(&magic, ext.as_deref()) {
        return Err(Error::unsupported(
            "PDF input is handled by the workspace build, not the standalone oxideav-io facade",
        ));
    }
    if is_mesh3d(ext.as_deref()) {
        return Err(Error::unsupported(
            "3D model input is handled by the workspace build, not the standalone oxideav-io facade",
        ));
    }
    // Everything else through the container+codec registry.
